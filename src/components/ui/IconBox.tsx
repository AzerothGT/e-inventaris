...
const IconBox = React.forwardRef<HTMLDivElement, IconBoxProps>(
	(
		{ className, icon: Icon, variant = "primary", size = 20, ...props },
		ref,
	) => {
		const variants = {
			...
			surface:
				"bg-surface-100/70 border border-surface-200/50 text-surface-600",
		};
		...
		return (
			<div
				ref={ref}
				className={cn(
					"flex shrink-0 items-center justify-center rounded-lg p-2 transition-colors duration-200",
					variants[variant],
					className,
				)}
				{...props}
