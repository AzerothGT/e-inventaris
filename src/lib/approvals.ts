...
> = {
	...
	selesai: {
		label: "selesai",
		color: "bg-success-500 text-white border-success-600",
	},
	ditolak: {
		label: "ditolak",
...
export function getAvailableActions(
	...
	return APPROVAL_CONFIG.filter(
		(action) => action.from === status && action.roles.includes(role),
	);
}
...
export function isValidTransition(
	...
	return APPROVAL_CONFIG.some(
		(action) =>
			action.from === from && action.to === to && action.roles.includes(role),
	);
}
