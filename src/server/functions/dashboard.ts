...
import type { PermintaanStatus, UserRole } from "../../lib/approvals";
...
export const getRecentActivity = createServerFn({ method: "GET" }).handler(
	async () => {
		...
		const query = db
			...
			.leftJoin(
				pengadaanEvent,
				eq(approvalLogs.permintaanId, pengadaanEvent.id),
			)
...
export const getLowStockItems = createServerFn({ method: "GET" }).handler(
	async () => {
		const session = await getAuthSession();
		if (!session) throw new Error("Unauthorized");
...
export const getIncomingProcurements = createServerFn({
	method: "GET",
}).handler(async () => {
	const session = await getAuthSession();
	if (!session) throw new Error("Unauthorized");
...
export const getTrendData = createServerFn({ method: "GET" }).handler(
	async () => {
		const session = await getAuthSession();
		if (!session) throw new Error("Unauthorized");
		...
		const submitted = await db
			...
			.from(pengadaanEvent)
			.where(
				sql`${pengadaanEvent.createdAt} >= ${Math.floor(sevenMonthsAgo.getTime() / 1000)}`,
			)
			.groupBy(
				sql`strftime('%Y-%m', datetime(${pengadaanEvent.createdAt}, 'unixepoch', 'localtime'))`,
			);
		...
		const selesai = await db
			...
			.groupBy(
				sql`strftime('%Y-%m', datetime(${approvalLogs.createdAt}, 'unixepoch', 'localtime'))`,
			);
		...
		return months.map(({ label, totalSubmitted, totalSelesai }) => ({
			label,
			totalSubmitted,
			totalSelesai,
		}));
