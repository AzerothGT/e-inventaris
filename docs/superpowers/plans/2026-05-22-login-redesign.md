# Login Page Redesign — Split-Screen Layout Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Redesign the login page from a centered single card to a split-screen layout with a branded left panel (dot grid pattern) and a glassmorphism login form on the right.

**Architecture:** Replace the current centered card layout in `login.tsx` with a responsive split-screen: left panel (50% on desktop) with primary blue background + dot grid pattern + branding, right panel (50%) with light gray background + centered glassmorphism card containing the login form. On mobile, the brand panel collapses to a top banner.

**Tech Stack:** React 19, TypeScript, Tailwind CSS v4, lucide-react, @tanstack/react-router, sonner

---

### Task 1: Rewrite login.tsx with split-screen layout

**Files:**
- Modify: `src/routes/login.tsx` (complete rewrite of JSX layout)

- [ ] **Step 1: Replace the entire return JSX in LoginPage with the split-screen layout**

The file structure stays the same — imports, Route definition, state management, and handleSubmit remain unchanged. Only the return JSX is replaced.

Replace the current `return (...)` block (lines 35-159) with:
...
Run: `pnpm dev`
Expected: Vite starts successfully on port 3000 with no TypeScript errors

- [ ] **Step 3: Visual check in browser**

Open http://localhost:3000/login
Expected: Split-screen layout visible on desktop, brand panel on left with dot grid, form card on right. On mobile width (<768px), brand panel hidden, form fills screen.
